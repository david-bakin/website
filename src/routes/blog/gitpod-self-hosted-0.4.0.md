...
Gitpod Self-Hosted runs on Kubernetes and is easily installed via "helm install". On gitpod.io you can find detailed documentation and some automation to install Gitpod on [vanilla Kubernetes](/docs/self-hosted) or [on Google Cloud Platform](/docs/self-hosted). For the next release, we plan to add support for AWS, Azure, and OpenShift.
...
The fastest way to get started is by installing Gitpod Self-Hosted [on Google Cloud Platform](/docs/self-hosted). There is also more in-depth documentation available on how to install Gitpod on [vanilla Kubernetes](/docs/self-hosted).
