...
In addition we have introduced multiple lifecycle phases for the tasks, such as [init](../../config-start-tasks#init-command) and [before](../../config-start-tasks#before-command). The main purpose is to avoid rerunning unnecessary commands such as full builds, when restarting workspaces or opening snapshots.
...
Check out [the docs](../../config-start-tasks) to learn more about pre-configuring things in Gitpod.
...
More information about writing `.gitpod.yml` configurations can be found in [the docs](../../configure).
