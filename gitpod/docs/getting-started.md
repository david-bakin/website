...
1. In the browser's address bar, prefix the entire URL with `gitpod.io/#` and press Enter.
   ...
   - We recommend you [install the Gitpod browser extension](./browser-extension) to make this a one-click operation.
...
1. Run `gp init` to generate a scaffold `.gitpod.yml` configuration file (see the [.gitpod.yml reference](./config-gitpod-file) for all configuration options).
...
Excellent! To learn more about the difference between `init` and `command`, please have a look at [Start Tasks](./config-start-tasks).
...
1. He opens a Gitpod workspace for that given pull request (learn more about [contexts](./context-urls)).
...
Depending on your Git provider, the steps to configure prebuilds vary slightly. Please refer to the dedicated [Prebuilds](./prebuilds) page for detailed instructions on how to configure prebuilds for your project.
...
- [Configure](./configure) to learn more about start tasks, environment variables and how to provide your own custom Docker image as the foundation for workspaces.
- [Develop](./develop) to learn more about the productivity gains you get with ephemeral workspaces, how contexts assist you and how to collaborate with team members.
